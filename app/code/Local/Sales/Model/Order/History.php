...
class Sales_Model_Order_History extends Core_Model_Abstract
{
    ...
    public function cancelRequest($data)
    {
        ...
        Mage::getModel('sales/order_history')
            ...
            ->addData('from_status', $toStatus)
            ->save();
        echo '<script>alert("Successfully send cancel request")</script>';
        echo "<script>location.href='" . Mage::getBaseUrl() . '/sales/quote/list' . "'</script>";
    }
    public function rejectedHistory($data)
    {
        ...
        Mage::getModel('sales/order_history')
            ...
            ->addData('from_status', $toStatus)
            ->save();
        echo "<script>location.href='" . Mage::getBaseUrl() . '/admin/order_history/list' . "'</script>";
    }
}
